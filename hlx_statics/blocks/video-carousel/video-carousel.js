...
export default async function decorate(block) {
  ...
  const video_carousel_block_child = createTag("div", {
    class: "video-block-container",
  });
  block.append(video_carousel_block_child);

  const video_carousel_block_circle = createTag("div", {
    class: "video-carousel-circle-div",
  });
  block.append(video_carousel_block_circle);

  const video_carousel_section = createTag("section", {
    class: "video-slider-wrapper",
  });
  video_carousel_block_child.append(video_carousel_section);

  const video_arrow_button_previous = createTag("button", {
    class: "video-slide-arrow",
  });
  ...
  const video_arrow_button_forward = createTag("button", {
    class: "video-slide-arrow",
  });
  ...
  const video_carousel_ul = createTag("ul", {
    class: "video-slides-container",
  });
  ...
  video_carousel_block_child.insertBefore(
    video_arrow_button_previous,
  ...
  videoNextButton.addEventListener("click", () => {
    isPaused = true;
    const slide_selected = block.querySelector(
      ".video-carousel-circle-selected"
    );
    const slide_selected_num = parseInt(slide_selected.id);
    const new_slide_num =
      slide_selected_num === count - 1 ? 1 : slide_selected_num + 1;
    const slideDx =
      videoSlidesContainer.clientLeft +
      videoSlide.clientWidth * (new_slide_num - 1);
  ...
  videoPrevButton.addEventListener("click", () => {
    ...
    const slide_selected = block.querySelector(
      ".video-carousel-circle-selected"
    ); //should only be one in the block
    const slide_selected_num = parseInt(slide_selected.id);
    const new_slide_num =
  ...
  function loadUrl(url) {
    ...
    const autoPlay =
      block?.parentElement?.parentElement?.getAttribute("data-autoplay");
    ...
    if (youtubeRegex.test(url)) {
      ...
      dataSource =
        autoPlay === "true" ? dataSource + "&autoplay=1&mute=1" : dataSource;
  ...
  function advanceSlide() {
    ...
    if (new_slide_num === count) {
      ...
      const slideDx =
        videoSlidesContainer.clientLeft +
        videoSlide.clientWidth * slide_selected_num;
      videoSlidesContainer.scrollLeft = slideDx;
      //change color of circle
